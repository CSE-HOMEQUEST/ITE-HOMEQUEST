...
## VI. Conclusion: Discussion(things to improve)


### 1. Introducing User Embeddings

In the current system, the userId is treated purely as a categorical variable, meaning the model distinguishes users only by their ID without understanding any behavioral similarity or preference patterns between them. This limits the system’s ability to personalize recommendations because individual tendencies are not numerically represented or learned. By introducing user embeddings, diverse signals such as past challenge completions, repeated preferences for certain categories, preferred activity time windows, and family interaction patterns can be compressed into a meaningful vector representation. Such embeddings enable the model to capture “what type of user someone is,” rather than simply identifying “who the user is,” thereby enhancing the personalization depth. Additionally, users with similar behaviors naturally cluster together in the embedding space, which allows the system to offer more stable recommendations to new users through similarity-based inference. Ultimately, embedding-based representations would provide a stronger foundation for scaling personalized recommendations within the HomeQuest environment.

### 2. Applying Time-Aware Sequence Models (LSTM · Transformer)

User behavior within HomeQuest is not static; it evolves over time and displays clear temporal dynamics that cannot be fully modeled by a purely static classifier. For example, challenge completion rates may drop during certain weeks, or weekend and weekday behaviors may diverge significantly, reflecting inherently shifting engagement patterns. Sequence models like LSTM are well suited for this setting, as they selectively retain short-term fluctuations and recent behavioral tendencies, allowing the system to incorporate immediate contextual changes. In contrast, Transformer-based models can capture long-range dependencies, periodic habits, seasonal variations, and behavioral shifts after specific events, offering a broader temporal perspective. Incorporating such models would move the system from simple “point-in-time probability estimation” toward learning the user’s behavioral trajectory itself. As a result, recommendations could reflect not only who the user is, but also how the user is currently changing, enabling more adaptive and context-sensitive challenge selection. Ultimately, time-aware sequence modeling represents a critical step toward transforming HomeQuest into a fully dynamic, context-responsive recommendation system.
